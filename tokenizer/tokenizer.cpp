            ...
            switch (current_state) {
                    ...
                    if (!current_char.has_value()) {
                        ...
                        if (re >= 0)
                            return std::make_pair(
                                    std::make_optional<Token>(TokenType::UNSIGNED_INTEGER, re, pos, currentPos()),
                                    std::optional<CompilationError>());
                        else
                            return std::make_pair(std::optional<Token>(),
                                                  std::make_optional<CompilationError>(0, 0,
                ...
                case IDENTIFIER_STATE: {
                    ...
                    if (!current_char.has_value()) {
                        ...
                        return std::make_pair(
                                std::make_optional<Token>(tokenType, ss.str(), pos, currentPos()),
                                std::optional<CompilationError>());
